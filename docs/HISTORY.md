...
too. That worked for sshd and for nothing else — `cydia`,
`com.firecore.freemem-watcher`, `com.nito.tssagent` and
`org.tihmstar.fuzzyparrot` were all equally dead on a tether boot, and every
package added later would have needed the same hand-treatment.

What ships instead is `xyz.regulad.blackb0x.loaddaemons`: a unit in the one
directory launchd does scan, running `/usr/share/blackb0x/loaddaemons.sh`,
which is the untether's own loop — load every plist in
`/Library/LaunchDaemons`, then run every script in `/etc/rc.d` — on every boot
rather than only on one where the exploit succeeded. It is idempotent because
it has to be: on an untethered boot the untether does all of this again, and
`daemonload` does the `/Library/LaunchDaemons` half a third time. `launchctl
load` of an already-loaded job fails and changes nothing.

**And the socket override was reverted, because the theory behind it was
wrong on both counts.** It had replaced openssh's plist to drop
`inetdCompatibility` and `SockServiceName "ssh"`, on the reasoning that the
first was vestigial on an XPC-era launchd and the second could not be resolved
without an `/etc/services` this project controls. Both were checked against
the real image and both are false: the device's `/sbin/launchd` contains
`inetdCompatibility`, `SockServiceName`, `Sockets` and the error string
`inetd-compatible service with no sockets`, and stock `/etc/services` is
677,972 bytes and carries `ssh 22/tcp` and `ssh 22/udp`. openssh's plist was
never broken. It was never loaded. With the override gone, the `Replaces:
openssh` and the `hold` that existed only to protect it are gone too — the
package tree is now strictly smaller than before this investigation started.
