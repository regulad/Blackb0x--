...
static void printUsage(const char* argv0) {
    fprintf(stderr,
        ...
        "      Retry the bug setup in-process until the device reports it is\n"
        "      actually holding a download buffer, instead of pressing on with\n"
        "      the destructive stages when it is not. Default 1 (single shot,\n"
