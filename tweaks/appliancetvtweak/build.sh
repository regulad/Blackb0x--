...
# mode assertion, same ownership reasoning -- because it has the same shape:
...
# would be a main menu that looks exactly like it does today.
...
cp "$DYLIB" "$STAGING/Library/MobileSubstrate/DynamicLibraries/appliancetvtweak.dylib"

if [ ! -f "$STAGING/Library/MobileSubstrate/DynamicLibraries/appliancetvtweak.plist" ]; then
...
# etc/rc.d/blackb0x-appliance-icons MUST be executable: the untether runs that
# directory's contents as programs, not through a shell, so a 644 file there is
# skipped silently with no error anywhere to notice.
for f in DEBIAN/postinst DEBIAN/preinst DEBIAN/prerm DEBIAN/postrm \
         Library/MobileSubstrate/DynamicLibraries/appliancetvtweak.dylib \
         etc/rc.d/blackb0x-appliance-icons; do
    [ -f "$STAGING/$f" ] && chmod 755 "$STAGING/$f"
done

if command -v plutil >/dev/null 2>&1; then
    plutil -lint "$STAGING/Library/MobileSubstrate/DynamicLibraries/appliancetvtweak.plist" >/dev/null \
